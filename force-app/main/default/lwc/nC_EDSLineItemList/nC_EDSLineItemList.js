...
export default class NC_EDSLineItemList extends LightningElement {
    ...
    submitChange() {
      ...
      updateAllLineItems({ lineItemData: JSON.stringify(this.lineItemsList) }).then(
        ...
        error => {
          let errorMessage;
          if(error.body && error.body.pageErrors && error.body.pageErrors.length > 0 && error.body.pageErrors[0].message) {
          ...
          if(errorMessage.indexOf('insufficient access rights on object id') != -1) {
            errorMessage = 'insufficient permission on Invoice';
          }
    ...
    queryInvoiceLineItems() {
      ...
      getAllLineItems({ recordId: this.recordId }).then(
        result => {
          ...
          this.totalAmountToPay = Number(result.totalAmountToPay);
          ...
          for(var i in this.lineItemsList) {
            if(this.lineItemsList[i].amountBilledEditable == false && this.lineItemsList[i].amountToPayEditable == false) {
    ...
    //Update value in amount to pay
    updateAmountToPayValues(event) {
        let sumOfAmount = 0;
        for(var i=0; i<this.lineItemsList.length; i++){
          if(event.target.name === this.lineItemsList[i].invoiceLineItemId){
            this.lineItemsList[i].amountToPay = event.target.value;
          }
          sumOfAmount += Number(this.lineItemsList[i].amountToPay);
        }
        this.totalAmountToPay = sumOfAmount;
        if(this.isHearingAid && !this.statusReturned){
          if(this.maxTotalAmountAuthorized < this.totalAmountToPay){
            this.totalAmountInputClass = 'slds-has-error' ;
            this.showTotalAmountError = true;
          }else{
            this.totalAmountInputClass = '' ;
          ...
          if(this.maxTotalAmountAuthorizedReturned < this.totalAmountToPay){
            this.totalAmountInputClass = 'slds-has-error' ;
            this.showTotalAmountError = true;
          }else{
            this.totalAmountInputClass = '' ;
